...
class MrpProductionBatchImportWizard(models.TransientModel):
    _name = "mrp.production.batch.import.wizard"
    _description = "Mrp Production Batch Import Wizard"
    ...
    line_ids = fields.One2many(
        "mrp.production.batch.import.line",
        "wizard_id",
        string="Lines",
    )
    ...
    def action_apply_lines(self):
        ...
        for line in lines_to_add:
            ...
            # Try to set locations using production's configured locations when present.
            if "location_src_id" in self.production_id._fields:
                vals["location_id"] = self.production_id.location_src_id.id
            if "location_dest_id" in self.production_id._fields:
                vals["location_dest_id"] = self.production_id.location_dest_id.id
            if "company_id" in self.production_id._fields and self.production_id.company_id:
                vals["company_id"] = self.production_id.company_id.id
        ...
        self.production_id.write({"move_raw_ids": move_vals})
...
class MrpProductionBatchImportLine(models.TransientModel):
    _name = "mrp.production.batch.import.line"
    _description = "Mrp Production Batch Import Line"

    wizard_id = fields.Many2one(
        "mrp.production.batch.import.wizard", required=True, ondelete="cascade"
    )
