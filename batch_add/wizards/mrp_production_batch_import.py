...
class MrpProductionBatchImportWizard(models.TransientModel):
    ...
    def action_apply_lines(self):
        ...
        move_vals = [
            (0, 0, self._prepare_component_move_vals(line.product_id, line.quantity))
            for line in lines_to_add
        ]
        self.production_id.write({"move_raw_ids": move_vals})
        return {"type": "ir.actions.act_window_close"}

    def _prepare_component_move_vals(self, product, quantity):
        vals = {
            ...
            "location_dest_id": self.production_id.production_location_id.id or self.production_id.location_dest_id.id,
...
class MrpProductionBatchImportLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        "mrp.production.batch.import.wizard",
        required=True,
        ondelete="cascade",
    )
