...
class SaleOrderBatchImportWizard(models.TransientModel):
    ...
    def _get_excel_sheet_names(self):
        ...
        if filename.endswith(".xls"):
            # If it's HTML-in-.xls, we treat it as single-sheet.
    ...
    def _parse_excel(self):
        ...
        if filename.endswith('.xlsx'):
            ...
            if requested:
                ...
                # Default to last sheet (many exports put the data on the last tab).
            ...
            try:
                ...
                # Some ".xls" files are actually HTML tables (common with exports).
                if self._looks_like_html_table(decoded):
    ...
    def _detect_qty_key(self, headers):
        ...
        # Prefer explicit quantity headers, and avoid 'amount' (often monetary).
        ...
        # Fallback: look for token matches but exclude obvious monetary columns.
        ...
        # Last resort: common shorthand seen in some exports (e.g. "s" for "sold"/"sum"/"qty").
        for header in normalized:
    ...
    def _build_descriptor(self, row, headers, descriptor_keys, qty_key):
        ...
        for key in descriptor_keys:
            ...
            if not value:
                continue
            # Skip pure numbers (these often represent qty/price/amount even if header is weird).
    ...
    def _search_candidates(self, descriptor):
        ...
        return Product.search([('sale_ok', '=', True)], limit=10)
    ...
    def _extract_size_token(self, text):
        if not text:
            return None
        text = str(text).lower()
        # Normalize common separators like "50 ml" -> "50ml"
