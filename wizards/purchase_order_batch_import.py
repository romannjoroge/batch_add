...
class PurchaseOrderBatchImportWizard(models.TransientModel):
    ...
    def action_parse_file(self):
        ...
        for index, row in enumerate(rows, start=1):
            descriptor = self._build_descriptor(
                row, headers, descriptor_keys, qty_key
            )
    ...
    def action_apply_lines(self):
        ...
        lines_to_add = self.line_ids.filtered(
        ...
        for line in lines_to_add:
            order_lines.append((0, 0, self._prepare_po_line_vals(
    ...
    # ------------------------------------------------------------------
    # Purchase line preparation (use onchanges to align with purchase UI)
    # ------------------------------------------------------------------
    ...
    def _prepare_po_line_vals(self, product, quantity):
        ...
        qty_field = 'product_qty' if 'product_qty' in PurchaseLine._fields else None
        if not qty_field:
            raise UserError(
                "Unsupported Purchase Order Line quantity field on this database."
            )
        vals[qty_field] = quantity

        uom_field = None
        for candidate in ('product_uom', 'product_uom_id'):
            if candidate in PurchaseLine._fields:
                uom_field = candidate
                break
        if uom_field:
        ...
        # Use a "new" record + onchanges so price/taxes/description match UI logic.
    ...
    # ------------------------------------------------------------------
    # File parsing (copied from sale import for consistent UX)
    # ------------------------------------------------------------------
        ...
        if filename.endswith('.xlsx'):
            ...
            if requested:
                if requested in wb.sheetnames:
                    ...
                    wb.close()
                    raise UserError(
                        f"Sheet '{requested}' was not found in this workbook. "
    ...
    def _detect_descriptor_keys(self, headers, qty_key):
        ignore_tokens = ('qty', 'quantity', 'qtty', 'amount', 'count', 'price', 'unit price', 'rate', 'total', 'value')
        keys = []
    ...
    def _match_product(self, row, headers, descriptor):
        ...
        for product in candidates:
            score = self._similarity_score(descriptor, product, descriptor_size)
    ...
    def _search_candidates(self, descriptor):
        ...
        return Product.search([('purchase_ok', '=', True)], limit=10)
    ...
    def _extract_size_token(self, text):
        ...
        text = str(text).lower()
        text = re.sub(
            r'(\d)\s+(ml|g|kg|oz|l|litre|liter|lb|lbs)',
            r'\1\2',
            text
        )
